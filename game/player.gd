	...
	for col_idx in get_slide_collision_count():
		...
		if shape is CollisionShape3D:
			if (shape as CollisionShape3D).shape is WorldBoundaryShape3D && bounds_ignore <= 0.0:
				take_damage(maxf(max_health.value * 0.2, health * 0.5), col.get_normal() * 16.0)
				bounds_ignore = 0.1
		
		var colc := col.get_collider()
		if colc is RigidBody3D:
			if colc.freeze: continue
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if !is_spectator:
		...
		if event.is_action_pressed(&"reload") && gun.reload == 0: gun.reload = procgun.reload_time.value; return
